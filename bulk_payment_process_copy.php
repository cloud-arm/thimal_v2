...
$result = $db->prepare("SELECT sum(pay_amount) FROM credit_payment WHERE  pay_id=:id   ");
$result->bindParam(':id', $pay_id);
$result->execute();
for ($i = 0; $row = $result->fetch(); $i++) {
...
if ($chq_amount == $pay_tot) {
  ...
  for ($i = 0; $row = $result->fetch(); $i++) {
    ...
    if ($type == "qb") {
      ...
      $ql = $db->prepare($sql);
      $ql->execute(array($invoice_no, 'credit', $date, $time, $pay_amount, $cus_balance, $sales_id, $customer_id, $user_id, $user_name));
    }
