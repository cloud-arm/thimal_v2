...
if (true) {
    ...
    //checking duplicate
    $con = 0;
    $result = $db->prepare("SELECT * FROM sales_list WHERE invoice_no = '$invoice' AND product_id = '$pid' AND date = '$date' ");
    ...
    if ($con == 0) {
        ...
        $ql = $db->prepare($sql);
        $ql->execute(array($invoice, $pid, $name, $amount, $cost_amount, $discount, $qty, $price, $profit, $date, $load, 0, $cus, $price_id, $vat, $value, $commission));


        header("location: sales_temp.php?id=$invoice");
    } else {
        $err = 2;

        header("location: sales_temp.php?id=$invoice&err=$err");
    }
} else {
    $err = 1;
