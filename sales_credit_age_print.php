...
<body onload="window.print() " style=" font-size: 13px; font-family: arial;">
  ...
  $sec = "1";
  ?>
  <meta http-equiv="refresh" content="<?php echo $sec; ?>;URL='sales_credit.php?type=<?php echo $_GET['type'] ?>&cus=<?php echo $_GET['cus'] ?>&lorry=<?php echo $_GET['lorry'] ?>&group=<?php echo $_GET['group'] ?>&customer_type=<?php echo $_GET['customer_type'] ?>'">
  <div class="wrapper">
    <!-- Main content -->
        ...
        <table id="example2" class="table table-bordered table-striped">
          ...
          <tbody>
            ...
            for ($i = 0; $row_cus = $customer->fetch(); $i++) {
              ...
              $result2z = $db->prepare("SELECT * FROM payment WHERE action='2' and type='credit' and credit_balance > 0 and customer_id='$cus'");
              ...
              for ($i = 0; $row = $result2z->fetch(); $i++) {
                ...
                if ($lorry == 'all') {
                  $result2 = $db->prepare("SELECT * FROM sales WHERE action='1' AND invoice_number='$sales_id'");
                } else {
                  $result2 = $db->prepare("SELECT * FROM sales WHERE action='1' AND invoice_number='$sales_id' AND lorry_no='$lorry' ");
                }
            ...
            ?>
                    <tr style="background-color: <?php echo $color; ?>; color: <?php echo $color1; ?>">
                      ...
                      <td><?php if ($due <= 30) {
                            echo number_format($row2['amount'] - $row['pay_amount'], 1);
                            $due30 += $row['amount'] - $row['pay_amount'];
                            $due30tot += $row['amount'] - $row['pay_amount'];
                          } ?></td>

                      <td><?php if ($due > 30 && $due <= 60) {
                            echo number_format($row2['amount'] - $row['pay_amount'], 1);
                            $due60 += $row['amount'] - $row['pay_amount'];
                            $due60tot += $row['amount'] - $row['pay_amount'];
                          } ?></td>

                      <td><?php if ($due > 60 && $due <= 90) {
                            echo number_format($row2['amount'] - $row['pay_amount'], 1);
                            $due90 += $row['amount'] - $row['pay_amount'];
                            $due90tot += $row['amount'] - $row['pay_amount'];
                          } ?></td>

                      <td><?php if ($due > 90) {
                            echo number_format($row2['amount'] - $row['pay_amount'], 1);
                            $due100 += $row['amount'] - $row['pay_amount'];
                            $due100tot += $row['amount'] - $row['pay_amount'];
                          } ?></td>
                    </tr>
                <?php
                  }
